  ...
  describe "subrecord CRUD" do
   ...
   it "deletes agent subrecords when parent agent is deleted" do
    ...
    it "updates subrecords along with agent" do
      ...
      new_json = build(:json_agent_software_full_subrec)
      ...
      expect(AgentRecordControl.where(:agent_software_id => agent_id).first[:maintenance_agency]).to eq(new_json["agent_record_controls"][0]["maintenance_agency"])
      expect(AgentAlternateSet.where(:agent_software_id => agent_id).first[:descriptive_note]).to eq(new_json["agent_alternate_sets"][0]["descriptive_note"])
      expect(AgentConventionsDeclaration.where(:agent_software_id => agent_id).first[:descriptive_note]).to eq(new_json["agent_conventions_declarations"][0]["descriptive_note"])
      expect(AgentOtherAgencyCodes.where(:agent_software_id => agent_id).first[:maintenance_agency]).to eq(new_json["agent_other_agency_codes"][0]["maintenance_agency"])
      expect(AgentMaintenanceHistory.where(:agent_software_id => agent_id).first[:descriptive_note]).to eq(new_json["agent_maintenance_histories"][0]["descriptive_note"])
      expect(AgentRecordIdentifier.where(:agent_software_id => agent_id).first[:record_identifier]).to eq(new_json["agent_record_identifiers"][0]["record_identifier"])
      expect(AgentSources.where(:agent_software_id => agent_id).first[:descriptive_note]).to eq(new_json["agent_sources"][0]["descriptive_note"])
    end 
  end
end
